...
describe Succubus::Grammar do
  ...
  describe "complex grammar" do
    it "should not expand escaped groups" do
      grammar = Succubus::Grammar.new do
        add_rule :base, '\<John> has a <colour> \<fluffy> <pet> called \<Fred>'
        add_rule :colour, "black", "white"
        add_rule :pet, "cat", "dog"
      end

      1000.times { grammar.execute(:base).must_match(/<John> has a (black|white) <fluffy> (cat|dog) called <Fred>/) }
    end

    it "should expand multiply-escaped groups only if the amount of escaping is even" do
      grammar = Succubus::Grammar.new do
        rule = <<'DATA'
\<John> has a \\<colour> \\\\\<fluffy> \\\\<pet> called \\\<Fred>
DATA
        add_rule :base, rule.chomp
        add_rule :colour, "black", "white"
        add_rule :pet, "cat", "dog"
      end
    ...
    it "should never unescape backslashes elsewhere" do
      grammar = Succubus::Grammar.new do
        add_rule :base, "I \\have \\\\ a <colour> \\\\\\ <pet>"
        add_rule :colour, "black", "white"
        add_rule :pet, "cat", "dog"
      ...
      1000.times { grammar.execute(:base).must_match(/I \\have \\\\ a (black|white) \\\\\\ (cat|dog)/) }
    end
  end
